...
//set variables
let u = { dataver: 1, name: `Player_${ran(100, 899)}`, pts: [0, 0, 0], plays: 0, lives: 0, ver: "0.5 [BETA 4]", tasks: { count: 0, first_game: [0, 1, 0, 1], plays: [0, 10, 10], earn_total: [0, 200, 100], earn_game: [0, 25, 50], save_lives: [0, 30, 0] } }, ver = "0.5 [BETA 4]",
  form = $("form");
//user data checks
...
if (!u.popup) {
  let share = `<p>Do you want to help us grow bigger by getting involved in the community on <a href="https://facebook.com/MathQuizOnline">Facebook</a> or <a href="https://m.me/MathQuizOnline">Message Us</a> if you have questions</p><div><center><a href="https://facebook.com/MathQuizOnline/"><img src="icons/like.png" style="width: 128px" alt="Like us on Facebook"></a></center></div>`;
...
function eg(pts, end) {
  ...
  endText.html(`Hey <span class="grey-bg">${u.name}</span>,<br/>It looks like you ${etf()} question <span class='grey-bg'>${$q}</span> and got <span class='grey-bg'>${pts}</span> points and got <span class='grey-bg'>${lp()}</span> bonus points for lives!<br/><br/>And you have played <span class='grey-bg'>${u.plays}</span> time(s)!<br/>${nh()}<br/><br/>Your target score is <span class='grey-bg'>${fv("target")}</span> on <span class='grey-bg'>${dlist[n(fv("diff")) - 1]}</span> difficulty!`);
  u.pts[0] += lp();
  uhs();
}
...
let passed = [];
function ti(){
let tl = $(".task"), vals = [u.plays, u.plays, u.pts[0] || 0, u.pts[1] || 0, u.lives];
 tl.each(function(i){
  try {
...
function game() {
  $("#quit").attr("pts",1)
  $("#quit").click(function(){eg($(this).attr("pts"),1)})
  lives = 3;
  $q = 1;
  $pts = 0;
  pages(3);
